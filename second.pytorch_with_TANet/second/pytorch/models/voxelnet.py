...
from second.pytorch.models import middle, pointpillars, rpn, voxel_encoder, tanet
...
class VoxelNet(nn.Module):
    ...
    def loss(self, example, preds_dict, refine_weight = 5):
        ...
        if self.rpn_class_name == "PSA" or self.rpn_class_name == "RefineDet":
            ...
            return {
                ...
                "refine_dir_loss_reduced": refine_dir_loss,  # dir_loss,
    ...
    def forward(self, example, refine_weight=5):
        ...
        if self.training:
            return self.loss(example, preds_dict, refine_weight)
        else:
    ...
    def compute_predict(self, batch_box_preds, batch_cls_preds,
                        batch_dir_preds,  batch_anchors_mask, num_class_with_bg, meta_list):
        predictions_dicts = []
        post_center_range = None
