    ...
    try:
        while True:
            ...
            for example in dataloader:
                ...
                ret_dict = net_parallel(example_torch, refine_weight)
                ...
                if global_step % steps_per_eval == 0:
                    ...
                    for example in iter(eval_dataloader):
                        ...
                        if model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
                            coarse, refine = net(example)
                            detections_coarse += coarse
                            detections_refine += refine
                            detections = detections_coarse
                        else:
                    ...
                    if model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
                        ...
                        with open(result_path_step / "result_refine.pkl", 'wb') as f:
                            pickle.dump(detections_refine, f)
                        result_dict_refine = eval_dataset.dataset.evaluation(
                            detections_refine, str(result_path_step))
                        for k, v in result_dict_refine["results"].items():
                            model_logging.log_text("Evaluation {}".format(k), global_step)
                            model_logging.log_text(v, global_step)
                        model_logging.log_metrics(result_dict_refine["detail"], global_step)
...
def evaluate(config_path,
    ...
    if model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
        ...
        print("\n")
        print("After Refine:")
        result_dict_refine = eval_dataset.dataset.evaluation(detections_refine,
                                                      str(result_path_step))
        if result_dict_refine is not None:
            for k, v in result_dict_refine["results"].items():
                print("Evaluation {}".format(k))
                print(v)
